const iplList = [];
function addIpl(number, ocPol, offender, victim, witness) {
    const ipl = {
        number,
        ocPol,
        offender,
...
function findIpl(number) {
    let ipl;
    ipl = iplList.find((x) => x.number === number);
    return ipl;
}
function addOffender(member, ipl) {
    ipl.offender.push(member);
}
function addVictim(member, ipl) {
    ipl.victim.push(member);
}
function addWitness(member, ipl) {
    ipl.witness.push(member);
}
function upDateParts(memberOff, memberVic, memberWit, ipl) {
